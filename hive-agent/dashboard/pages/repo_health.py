...
def render_viz22_health_scorecard(
    ...
    metrics = [
        ...
        (
            "AVG COMPLEXITY",
            f"{avg_complexity:.1f}",
            _traffic_light(avg_complexity, 5, 10, higher_is_better=False),
        ),
        (
            "SECURITY ISSUES (HIGH)",
            str(high_security),
            _traffic_light(high_security, 0, 5, higher_is_better=False),
        ),
        (
            "CLONE GROUPS",
            str(clone_count),
            _traffic_light(clone_count, 3, 10, higher_is_better=False),
        ),
        (
            "OPEN ISSUES",
            str(open_issues),
            _traffic_light(open_issues, 10, 50, higher_is_better=False),
        ),
    ...
    grade = (
        "A"
        if pct >= 0.9
        else "B" if pct >= 0.8 else "C" if pct >= 0.7 else "D" if pct >= 0.6 else "F"
    )
        ...
        [
            ...
            dbc.Row(
                [
                    ...
                    dbc.Col(
                        [html.Div(metric_cards, style={"display": "flex", "flexWrap": "wrap"})],
                        width=10,
                    ),
                ]
            ),
