...
def render_viz20_before_after(doc_quality_scores: Optional[Dict] = None) -> html.Div:
    ...
    fig.add_trace(
        go.Bar(
            name="Before HIVE-AGENT",
            x=["Coverage", "BERTScore"],
            y=[before * 100, 0.0],
    ...
    fig.add_trace(
        go.Bar(
            name="After HIVE-AGENT",
            x=["Coverage", "BERTScore"],
            y=[after * 100, bertscore_after * 100],
            marker_color=SUCCESS,
        )
    )
    fig.update_layout(
        title="Before vs After HIVE-AGENT Documentation",
        barmode="group",
        yaxis_title="Score (%)",
        **PLOTLY_THEME,
    )
    ...
    return html.Div(
        [
            ...
            html.Div(
                f"Coverage improvement: +{delta_cov*100:.1f}%",
                style={
                    "color": SUCCESS,
                    "fontFamily": FONT,
                    "fontSize": "12px",
