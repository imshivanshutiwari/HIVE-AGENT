...
from dashboard.theme import (
    ...
    WARNING, CRITICAL, INFO, TEXT, BORDER, FONT, PLOTLY_THEME, CARD_STYLE,
)
        ...
        dbc.Row([
            ...
            dbc.Col([
                html.Div(
                    ...
                    children=["[HIVE-AGENT] Ready. Enter a GitHub repo URL to begin analysis."],
                ),
            ], width=6),
        ]),
...
def render_viz02_repo_overview(metadata: Optional[Dict] = None) -> html.Div:
    ...
    if not metadata:
        return html.Div([
            ...
            dcc.Graph(figure=make_empty_fig("Awaiting repository data..."), id="viz02-lang-pie"),
        ], style=CARD_STYLE)
    ...
    stat_cards = [
        html.Div([
            ...
            html.Div(value, style={"color": ACCENT_BLUE, "fontSize": "16px", "fontWeight": "bold"}),
...
def render_viz05_doc_preview(generated_docs: Optional[Dict] = None) -> html.Div:
    ...
    if generated_docs:
        for key, doc in list(generated_docs.items())[:5]:
            ...
            entries.append(html.Div([
                html.Div(name, style={"color": ACCENT_BLUE, "fontSize": "11px", "marginBottom": "4px"}),
                html.Pre(doc[:400], style={
                    "backgroundColor": BG_PRIMARY,
                    "color": SUCCESS,
                    "fontFamily": FONT,
                    "fontSize": "10px",
    ...
    if not entries:
        entries = [html.Div("Awaiting generated documentation...", style={"color": TEXT, "fontFamily": FONT})]
...
def render_viz06_generated_tests(generated_tests: Optional[Dict] = None) -> html.Div:
    ...
    if generated_tests:
        for key, test in list(generated_tests.items())[:3]:
            name = key.split("/")[-1]
            entries.append(html.Div([
                html.Div(name, style={"color": SUCCESS, "fontSize": "11px", "marginBottom": "4px"}),
                html.Pre(test[:500], style={
                    "backgroundColor": BG_PRIMARY,
                    "color": TEXT,
                    "fontFamily": FONT,
                    "fontSize": "10px",
    ...
    test_count_fig = go.Figure(go.Bar(
        x=[k.split("/")[-1][:20] for k in list(generated_tests.keys())[:10]] if generated_tests else ["No tests yet"],
    ...
    if not entries:
        entries = [html.Div("Awaiting test generation...", style={"color": TEXT, "fontFamily": FONT})]
...
def render_viz07_agent_log(
    pipeline_trace: Optional[List] = None, tokens_used: int = 0
) -> html.Div:
        ...
        dbc.Row([
            dbc.Col([
                html.Div(
                    ...
                    style={"color": ACCENT_BLUE, "fontFamily": FONT, "fontSize": "12px"},
                ),
            ], width=4),
            dbc.Col([
                html.Div(
